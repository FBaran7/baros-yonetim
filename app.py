...
if sayfa == "Ana Panel":
    ...
    st.caption("Depoda ürün kontrolü, barkod okuma ve hızlı görsel kayıt için yardımcı ekran")

    st.warning(
        'Kamera mobilde açılmıyorsa: Telefon tarayıcıları güvenlik gereği kamerayı sadece "https://" bağlantılarda açar. Yerel ağ (http) testlerinde kamera engellenebilir. Uygulama buluta yüklendiğinde bu sorun çözülecektir.'
    )

    manuel_barkod = st.text_input("Barkod numarasını manuel girin (opsiyonel)")
    kamera_gorseli = st.camera_input("Barkod / ürün etiketi fotoğrafı çekin")

    aktif_barkod = None

    if manuel_barkod.strip():
        aktif_barkod = manuel_barkod.strip()
        st.success(f"Manuel barkod alındı: {aktif_barkod}")
        ...
        try:
            ...
            if decoded_objects:
                ...
                st.success(f"Barkod başarıyla okundu: {aktif_barkod}")
            else:
    ...
    if aktif_barkod:
        ...
        if not stok_df.empty and "Ürün Kodu" in stok_df.columns:
            bulunan_urunler = stok_df[stok_df["Ürün Kodu"].astype(str) == str(aktif_barkod)].copy()

            if not bulunan_urunler.empty:
                bulunan_gosterim = stok_tablo_gosterime_hazirla(bulunan_urunler)
                st.subheader("Eşleşen Stok Kaydı")
                st.dataframe(bulunan_gosterim, use_container_width=True, hide_index=True)
            else:
                st.info("Bu barkoda ait ürün stok.csv içinde bulunamadı.")
        else:
            st.info("Stok verisi henüz hazır değil.")